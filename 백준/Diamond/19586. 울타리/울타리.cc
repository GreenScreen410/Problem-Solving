...
long double rotating_calipers(vector<Point>& hull) {
  ...
  long double min_perimeter = LLONG_MAX;
  for (int i = 0; i < n; i++) {
    if (p1 % n == i) p1++;
    if (p2 % n == i) p2++;
    if (p3 % n == i) p3++;
    ...
    long double width = point_to_line_distance(hull[i], hull[p2 % n], p);
    long double height = point_to_line_distance(hull[p1 % n], hull[p3 % n], {p.y, -p.x});
    min_perimeter = min(min_perimeter, width * 2 + height * 2);
  }
...
int main() {
  ...
  if (hull.size() == 2) {
    cout << euclidean_distance(hull[0], hull[1]) * 2;
    return 0;
  }
